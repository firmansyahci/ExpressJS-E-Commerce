...
module.exports = {
    ...
    productDetail: (id) => {
        return new Promise((resolve, reject) => {
            connection.query("SELECT product.*, categories.name as category_name FROM product LEFT JOIN categories ON product.category_id = categories.id WHERE product.id = ?", id, (err, result) => {
                if (!err) {
                    resolve(result);
                } else {
                    reject(new Error(err));
    ...
    sortProductDesc: () => {
        return new Promise((resolve, reject) => {
            connection.query("SELECT * FROM product ORDER BY name DESC", (err, result) => {
                if (!err) {
                    resolve(result);
                } else {
                    reject(new Error(err));
